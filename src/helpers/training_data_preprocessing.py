...
def get_data_tabm(data:pd.DataFrame=None,gene_cols:List[str]=None):
    numeric_cols, categorical_cols, datetime_cols = categorize_columns(data, gene_cols=gene_cols)
...
def categorize_columns(
    df, max_unique_count: int = 2, max_unique_ratio: float = 0.05, gene_cols: List[str] = None
) -> Tuple[List[str], List[str], List[str]]:
    df = df.convert_dtypes()
    n = len(df)
    ...
    num_low_card = [
        col for col in numeric
        if df[col].nunique(dropna=True) <= max_unique_count and col not in gene_cols
    ]
    numeric = [c for c in numeric if c not in num_low_card]
    categorical = sorted(set(categorical + num_low_card))
